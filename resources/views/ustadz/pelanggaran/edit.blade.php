{{-- resources/views/ustadz/pelanggaran/edit.blade.php --}}
@extends('layouts.app')
@section('title', 'Edit Pelanggaran')
@section('page-title', 'Edit Pelanggaran')
@section('breadcrumb', '/ <a href="' . route('ustadz.pelanggaran.index') . '">Pelanggaran</a> / <span>Edit</span>')
...
<div class="page-header">
    <div>
        <div class="page-header-title">Edit Pelanggaran</div>
        <div class="page-header-sub">Perbarui data pelanggaran santri</div>
    </div>
    <a href="{{ route('ustadz.pelanggaran.index') }}" class="btn-outline-hijau">← Kembali</a>
</div>

<form method="POST" action="{{ route('ustadz.pelanggaran.update', $pelanggaran) }}">
    @csrf @method('PUT')
            ...
            <div class="row g-3">
                    ...
                    <div class="form-group-custom">
                        ...
                        <select name="santri_id" class="form-control-custom" required>
                            ...
                            @foreach($santriList as $s)
                                <option value="{{ $s->id }}" {{ old('santri_id', $pelanggaran->santri_id) == $s->id ? 'selected' : '' }}>
                ...
                <div class="col-md-3">
                    <div class="form-group-custom">
                        ...
                        <input type="date" name="tanggal" class="form-control-custom"
                            value="{{ old('tanggal', $pelanggaran->tanggal->format('Y-m-d')) }}" max="{{ date('Y-m-d') }}" required>
                ...
                <div class="col-md-3">
                    <div class="form-group-custom">
                        ...
                        <input type="text" class="form-control-custom" value="{{ $pelanggaran->pencatat->name ?? Auth::user()->name }}" disabled>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    <div class="form-group-custom">
                        ...
                        <input type="number" name="poin_sanksi" id="poinSanksi" class="form-control-custom"
                            value="{{ old('poin_sanksi', $pelanggaran->poin_sanksi) }}" min="1" max="200" required>
                ...
                <div class="col-md-3">
                    <div class="form-group-custom">
                        ...
                        <input type="text" id="tingkatDisplay" class="form-control-custom" disabled placeholder="Otomatis">
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-12">
                    <div class="form-group-custom">
                        ...
                        <input type="text" name="jenis_pelanggaran" class="form-control-custom"
                            ...
                            placeholder="Contoh: Terlambat hadir shalat Subuh">
                ...
                <div class="col-md-8">
                    <div class="form-group-custom">
                        ...
                        <textarea name="keterangan" class="form-control-custom" rows="3"
                            placeholder="Deskripsi detail kejadian...">{{ old('keterangan', $pelanggaran->keterangan) }}</textarea>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group-custom">
                        ...
                        <select name="status_tindak_lanjut" class="form-control-custom" required>
                            <option value="belum" {{ old('status_tindak_lanjut', $pelanggaran->status_tindak_lanjut) === 'belum' ? 'selected' : '' }}>Belum</option>
                            <option value="sudah" {{ old('status_tindak_lanjut', $pelanggaran->status_tindak_lanjut) === 'sudah' ? 'selected' : '' }}>Sudah</option>
                        </select>
                    </div>
                    <div class="form-group-custom">
                        <label class="form-label-custom">Tindak Lanjut</label>
                        <input type="text" name="tindak_lanjut" class="form-control-custom"
                            value="{{ old('tindak_lanjut', $pelanggaran->tindak_lanjut) }}" placeholder="Catatan tindak lanjut...">
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateFromKategori() {
        ...
        // Only auto-fill poin if the field is empty or matches previous default
        if (!poinInput.value || poinInput.dataset.lastDefault === poinInput.value) {
            poinInput.value = poin;
        }
        poinInput.dataset.lastDefault = poin;
    ...
    kategoriSelect.addEventListener('change', function() {
        // On change, always update poin
        const opt = kategoriSelect.options[kategoriSelect.selectedIndex];
        if (opt && opt.value) {
            poinInput.value = opt.dataset.poin;
            poinInput.dataset.lastDefault = opt.dataset.poin;
        }
        updateFromKategori();
    });
    ...
    if (kategoriSelect.value) {
        poinInput.dataset.lastDefault = poinInput.value;
        updateFromKategori();
    }
});
</script>
